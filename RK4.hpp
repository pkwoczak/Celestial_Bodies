...
std::vector<double> one_iter(std::vector<double> &x, size_t n, size_t dim){
    // x = [pos1, pos2, ..., posn, vel1, vel2, ..., veln]
    ...
    for (size_t i = 0; i < n; i++){
        for (size_t j = i+1; j < n; j++){
            ...
            double acc = G*x[j + n*dim]*pow(dist, -3);
            // update acceleration of body i
            for (size_t k = 0; k < dim; k++){
                f[i + n*dim + k] += acc*(x[j*dim + k] - x[i*dim + k]);
            }
            // update acceleration of body j
            for (size_t k = 0; k < dim; k++){
                f[j + n*dim + k] += acc*(x[i*dim + k] - x[j*dim + k]);
            }
        }
    }
    return f;
