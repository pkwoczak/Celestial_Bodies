...
for (size_t i = 0; i < nsteps; i++) {
    // new position = 2 * position - old position + acceleration * timestep^2
    // iterate over good_indices
    for (size_t j = 0; j < good_indices.size(); j++){
        positions[good_indices[j] * dim] = 2 * positions[good_indices[j] * dim] - trajectory[i][good_indices[j]][0] + accelerations[good_indices[j] * dim] * timeStep * timeStep;
        positions[good_indices[j] * dim + 1] = 2 * positions[good_indices[j] * dim + 1] - trajectory[i][good_indices[j]][1] + accelerations[good_indices[j] * dim + 1] * timeStep * timeStep;
    }
